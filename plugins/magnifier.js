(function($) {
    ...
    $.Magnifier = function(options) {
        ...
        if ( viewer.showMagnifierControl ) {
            viewer.magnifierButton = new $.Button({
                element:    viewer.magnifierButton ? $.getElement( viewer.magnifierButton ) : null,
                clickTimeThreshold: viewer.clickTimeThreshold,
                clickDistThreshold: viewer.clickDistThreshold,
                tooltip:    $.getString( "Tooltips.Magnifier" ),
                srcRest:    prefix + viewer.navImages.magnifier.REST,
                srcGroup:   prefix + viewer.navImages.magnifier.GROUP,
                srcHover:   prefix + viewer.navImages.magnifier.HOVER,
                srcDown:    prefix + viewer.navImages.magnifier.DOWN,
                onRelease:  this.toggleVisibility.bind(this),
                onFocus:    onFocusHandler,
                onBlur:     onBlurHandler
            });
            if (useGroup) {
                viewer.buttons.buttons.push(viewer.magnifierButton);
                viewer.buttons.element.appendChild(viewer.magnifierButton.element);
            }
        }
